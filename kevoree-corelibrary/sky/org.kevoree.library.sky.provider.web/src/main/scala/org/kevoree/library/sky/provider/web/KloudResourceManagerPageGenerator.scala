...
abstract class KloudResourceManagerPageGenerator (instance: KloudResourceManagerPage, pattern: String) {
  ...
  val bootstrapCSSRequest = new Regex(pattern + "bootstrap/bootstrap.min.css")
  val bootstrapResponsiveCSSRequest = new Regex(pattern + "bootstrap/bootstrap-responsive.min.css")
  val bootstrapGlyphiconsWhiteRequest = new Regex(pattern + "bootstrap/glyphicons-halflings-white.png")
  val bootstrapGlyphiconsRequest = new Regex(pattern + "bootstrap/glyphicons-halflings.png")
  val bootstrapJSRequest = new Regex(pattern + "bootstrap/bootstrap.min.js")
  ...
  def process (request: KevoreeHttpRequest, response: KevoreeHttpResponse): KevoreeHttpResponse = {
    val processor = processBootstrapResources(request, response) orElse processJQueryResources(request, response) orElse processInternalResources(request, response) orElse internalProcess(request, response) orElse processError(request, response)
  ...
  protected def processBootstrapResources (request: KevoreeHttpRequest, response: KevoreeHttpResponse): PartialFunction[String, KevoreeHttpResponse] = {
    ...
    case bootstrapGlyphiconsRequest() => sendFile(request, response, getBytesFromStream(this.getClass.getClassLoader.getResourceAsStream("bootstrap/img/glyphicons-halflings-white.png")), "\"image/png\"")
    case bootstrapGlyphiconsWhiteRequest() => sendFile(request, response, getBytesFromStream(this.getClass.getClassLoader.getResourceAsStream("bootstrap/img/glyphicons-halflings.png")), "\"image/png\"")
