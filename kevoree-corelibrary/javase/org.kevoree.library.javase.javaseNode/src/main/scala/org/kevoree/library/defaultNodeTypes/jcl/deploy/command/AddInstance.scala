...
case class AddInstance(c: Instance, nodeName: String,modelservice : KevoreeModelHandlerService,kscript : KevScriptEngineFactory,bs : org.kevoree.api.Bootstraper) extends PrimitiveCommand {
  ...
  def execute(): Boolean = {
    ...
    try {
      ...
      if(newInstance.isInstanceOf[KevoreeComponentActivator]){
        newInstance.asInstanceOf[KevoreeComponentActivator].componentActor.getKevoreeComponentType.asInstanceOf[AbstractComponentType].setBootStrapperService(bs)
      }
      true
    } catch {
      case _@e => {
