...
public class WindowUtils
{
   ...
   */
  /*
   * public static void makeWindowNonOpaque(Window window) { // on the mac,
   * simply setting the window's background color to be fully transparent makes
   * // the window non-opaque. window.setBackground(new Color(0, 0, 0, 0)); //
   * on non-mac platforms, try to use the facilities of Java 6 update 10. if
   * (!PlatformUtils.isMac()) { quietlyTryToMakeWindowNonOqaque(window); } }
   */

  /**
   * Creates and installs a {@link WindowFocusListener} on the given
   * {@link Window} which calls the {@code Window}'s {@code repaint()} method on
   * focus state changes.
   * 
   * @param window
   ...
   * (i.e. it's not null) and that {@link Window} is currently active (focused).
   * 
   * @param component
   *          the {@code Component} to check the parent {@code Window}'s focus
   *          for.
   * @return {@code true} if the given {@code Component}'s parent {@code Window}
   *         is currently active.
   */
   ...
   * parent {@link Window}. If the {@code JComponent} doesn't yet have a parent,
   * then the listener will be installed when the component is added to a
   * container.
   * 
   * @param component
   *          the component who's parent frame to listen to focus changes on.
   * @param focusListener
  ...
  /**
   * Installs a listener on the given {@link JComponent}'s parent {@link Window}
   * that repaints the given component when the parent window's focused state
   * changes. If the given component does not have a parent at the time this
   * method is called, then an ancestor listener will be installed that installs
   * a window listener when the components parent changes.
   * 
   * @param component
  ...
  {
    if (component instanceof WindowFocusListener)
    {
      return (WindowFocusListener) component;
    }
    else
    {
      return new WindowFocusListener()
      {
        public void windowGainedFocus(WindowEvent e)
        {
          if (component instanceof WindowFocusListener)
          {
            ((WindowFocusListener) component).windowGainedFocus(e);
          }
          component.repaint();
        }
        ...
        public void windowLostFocus(WindowEvent e)
        {
          if (component instanceof WindowFocusListener)
          {
            ((WindowFocusListener) component).windowLostFocus(e);
          }
          component.repaint();
        }
      };
