...
abstract class KevoreeCoreBean(): KevoreeModelHandlerService {
    ...
    fun updateModel(model: ContainerRoot): Unit {
    ...
    fun start() {
        ...
        scheduler = java.util.concurrent.Executors.newSingleThreadExecutor(object : ThreadFactory{
            ...
            override fun newThread(p1: Runnable): Thread {
                val t = Thread(group, p1, "Kevoree_Core_Scheduler_" + getNodeName())
                if (t.isDaemon()) {
                    t.setDaemon(false)
                }
        ...
        if (nodeInstance != null) {
            try {
                ...
                if (stopModel != null) {
                    ...
                    val ignoredDeployResult = PrimitiveCommandExecutionHelper.execute(rootNode, adaptationModel, nodeInstance.sure(), afterUpdateTest, afterUpdateTest, afterUpdateTest)
                } else {
    ...
    fun checkModel(tModel: ContainerRoot): Boolean {
    ...
    fun updateModel(model: ContainerRoot, callback: ModelUpdateCallback) {
        scheduler?.submit(UpdateModelCallable(modelCloner.clone(model, true).sure(), callback))
    }

    fun compareAndSwapModel(previousModel: UUIDModel, targetModel: ContainerRoot, callback: ModelUpdateCallback) {
    ...
    fun compareAndSwapModel(previousModel: UUIDModel, targetModel: ContainerRoot): Unit {
        scheduler?.submit(CompareAndSwapCallable(previousModel, modelCloner.clone(targetModel, true).sure(), null))
    }

    fun atomicUpdateModel(model: ContainerRoot): Date {
    ...
    fun atomicCompareAndSwapModel(previousModel: UUIDModel, targetModel: ContainerRoot): Date {
    ...
    fun registerModelListener(listener: ModelListener) {
        modelListeners.addListener(listener)
    }

    fun unregisterModelListener(listener: ModelListener) {
    ...
    fun acquireLock(callBack: ModelHandlerLockCallBack, timeout: Long) {
        ...
        try {
            return if (nodeInstance != null) {
                ...
                if(foundNode != null){
                    ...
                    modelTmp.getNodes().filter{ n -> n.getName() != getNodeName() }.forEach { node ->
            ...
            if (nodeInstance == null) {
                ...
                if(foundNode != null){
                    ...
                    if(nodeInstance != null){
                        ...
                        model.getNodes().filter{ n -> n.getName() != getNodeName() }.forEach{ node ->
    ...
    private fun internal_update_model(proposedNewModel: ContainerRoot): Boolean {
        return if (proposedNewModel == null || proposedNewModel?.findByQuery("nodes[" + getNodeName() + "]", javaClass<ContainerNode>())!=null) {
            ...
            try {
                ...
                if ((!checkResult?.isEmpty().sure()) || (!versionCheckResult?.isEmpty().sure())) {
                    ...
                    if (preCheckResult && initUpdateResult) {
                        ...
                        if (hkh.detectNodeHaraKiri(model, readOnlyNewModel, getNodeName())) {
                            ...
                            try {
                                ...
                                PrimitiveCommandExecutionHelper.execute(rootNode, adaptationModel, nodeInstance.sure(), afterUpdateTest, afterUpdateTest, afterUpdateTest)
                                nodeInstance?.stopNode()
                                //end of harakiri
                                nodeInstance = null
                        ...
                        try {
                            ...
                            val preCmd = PreCommand()
                            val postRollbackTest : ()->Boolean = { () ->
                                modelListeners.postRollback(model, newmodel)
                                true
                            }
                            val rootNode = newmodel.findByQuery("nodes[" + getNodeName() + "]", javaClass<ContainerNode>()).sure()
                            deployResult = PrimitiveCommandExecutionHelper.execute(rootNode, adaptationModel, nodeInstance.sure(), afterUpdateTest, preCmd.preRollbackTest, postRollbackTest)
                        } catch(e: Exception) {
                            logger?.error("Error while update ", e)
                            deployResult = false
                        }
