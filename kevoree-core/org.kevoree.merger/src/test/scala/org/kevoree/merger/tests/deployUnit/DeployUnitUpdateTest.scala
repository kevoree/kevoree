...
class DeployUnitUpdateTest extends MergerTestSuiteHelper {
  ...
  @Test def verifyComponentTypeAdded() {
    val mergedModel = component.merge(model("deployUnit/beforeUpdate.kev"), model("deployUnit/afterUpdateNoContractChanged.kev"))
    mergedModel testSave ("deployUnit", "checkDeployUnitUpdateWihtoutContractChanged.art2")
        /*
