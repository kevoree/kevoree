        ...
        if (groupList.size() == 1) {
            if (groupList.get(0).equals("*")) {
                ...
                if (group != null) {
                    ...
                    throw new KevScriptError("Unable to find group instance \""+groupList.get(0)+"\". Attach failed");
                }
            }
        } else {
            throw new KevScriptError("Attach target path is invalid ("+groupList.toString()+")");
        }
            ...
            if (instancePath.size() == 1) {
                if (instancePath.get(0).equals("*")) {
                    ...
                    if (node != null) {
                        ...
                        throw new KevScriptError("Unable to attach node instance \""+instancePath.get(0)+"\". Instance does not exist");
                    }
                }
            } else {
                throw new KevScriptError("\""+instancePath.toString()+"\" is not a valid attach path for a node");
            }
        }
    }
