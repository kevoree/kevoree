...
<YYINITIAL> {
  ...
  "true"                         |
  "false"                        |
  "null"                         { return token(TokenTypes.KEYWORD); }
  ...
  "CharSequence"                 |
  "Thread"                       |
  "Regex"                        { return token(TokenTypes.TYPE); }
  ...
  "Comparable"                   |
  "Serializable"                 |
  "Runnable"                     { return token(TokenTypes.TYPE); }

  /* Groovy commonly used methods */
  "print"                        |
  "println"                      { return token(TokenTypes.KEYWORD); }
  ...
  "UnsupportedOperationException"    { return token(TokenTypes.TYPE2); }
  ...
  {FloatLiteral}                 |
  {DoubleLiteral}                |
  {DoubleLiteral}[dD]            { return token(TokenTypes.NUMBER); }
  ...
  /*
     ...
     a regular /
  */
  "/" [^*] {RegexCharacter}+ "/"  { return token(TokenTypes.REGEX); }
...
<STRING> {
  ...
  "${"                           { 
                                    ...
                                    tokenStart = yychar;
                                    tokenLength = 2;
                                    return token(TokenTypes.STRING, s, l);
                                 }
...
<GSTRING_EXPR> {
    "}"                          { 
                                    ...
                                    tokenStart = yychar + 1;
                                    tokenLength = 0;
                                    return token(TokenTypes.STRING2, s, l);
                                 }
...
<ML_STRING> {
  ...
  \"{3}                          {
                                    yybegin(YYINITIAL);
                                    // length also includes the trailing quote
                                    return token(TokenTypes.STRING, tokenStart, tokenLength + 3);
                                 }
...
<JDOC> {
  "*/"                           {
                                     yybegin(YYINITIAL);
                                     return token(TokenTypes.COMMENT, tokenStart, tokenLength + 2);
                                 }
  ...
  "@"                            {
                                     ...
                                     int len = tokenLength;
                                     tokenLength = 1;
                                     return token(TokenTypes.COMMENT, start, len);
                                 }
...
<JDOC_TAG> {
  ...
  .|\n                           {
                                     ...
                                     int len = tokenLength;
                                     tokenLength = 1;
                                     return token(TokenTypes.COMMENT2, start, len);
                                 }
}
...
<REGEX> {
  "/"                            { 
                                     yybegin(YYINITIAL); 
                                     // length also includes the trailing quote
                                     return token(TokenTypes.REGEX, tokenStart, tokenLength + 1);
                                 }
