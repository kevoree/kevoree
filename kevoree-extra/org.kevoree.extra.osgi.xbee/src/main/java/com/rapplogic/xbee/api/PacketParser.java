...
public class PacketParser implements IIntInputStream, IPacketParser {
	...
	private final static Logger log = Logger.getLogger(PacketParser.class.getName());
	...
	static void registerResponseHandler(int apiId, Class<? extends XBeeResponse> clazz) {
		if (handlerMap.get(apiId) == null) {
			log.info("Registering response handler " + clazz.getCanonicalName() + " for apiId: " + apiId);
			 ...
			 log.log(Level.WARNING,"Overriding existing implementation: " + handlerMap.get(apiId).getCanonicalName() + ", with " + clazz.getCanonicalName() + " for apiId: " + apiId);
		}
	...
	public XBeeResponse parsePacket() {
		...
		try {
			...
			this.length = new XBeePacketLength(msbLength, lsbLength);
			 ...
			 log.log(Level.FINEST,"packet length is " + ByteUtils.formatByte(length.getLength()));
			...
			for (Integer handlerApiId : handlerMap.keySet()) {
				if (intApiId == handlerApiId) {
					 log.log(Level.FINEST,"Found response handler for apiId [" + ByteUtils.toBase16(intApiId) + "]: " + handlerMap.get(handlerApiId).getCanonicalName());
			...
			// added bytes read for troubleshooting
			 log.log(Level.SEVERE,"Failed due to exception.  Returning ErrorResponse.  bytes read: " + ByteUtils.toBase16(rawBytes.getIntArray()), e);
			exception = e;
	...
	public int read(String context) throws IOException {
		int b = this.read();
		 log.log(Level.FINEST,"Read " + context + " byte, val is " + ByteUtils.formatByte(b));
		return b;
	}
	...
	public int read() throws IOException {
		...
		if (XBeePacket.isSpecialByte(b)) {
			 log.log(Level.FINEST,"Read special byte that needs to be unescaped");
			
			if (b == XBeePacket.SpecialByte.ESCAPE.getValue()) {
				 log.log(Level.FINEST,"found escape byte");
				// read next byte
				b = this.readFromStream();
				
				 log.log(Level.FINEST,"next byte is " + ByteUtils.formatByte(b));
				b = 0x20 ^ b;
				 log.log(Level.FINEST,"unescaped (xor) byte is " + ByteUtils.formatByte(b));
				...
				// while reading remote at response Found unescaped special byte base10=19,base16=0x13,base2=00010011 at position 5 
				 log.log(Level.WARNING,"Found unescaped special byte " + ByteUtils.formatByte(b) + " at position " + bytesRead);
			}
		}
		...
		if (bytesRead > 2) {
			...
			checksum.addByte(b);
			 ...
			 log.log(Level.FINEST,"Read byte " + ByteUtils.formatByte(b) + " at position " + bytesRead + ", packet length is " + this.length.get16BitValue() + ", #escapeBytes is " + escapeBytes + ", remaining bytes is " + this.getRemainingBytes());
			...
			if (this.getFrameDataBytesRead() >= (length.get16BitValue() + 1)) {
				// this is checksum and final byte of packet
                                ...
                                log.log(Level.FINEST,"Checksum byte is " + b);
	...
	public int[] readRemainingBytes() throws IOException {
		...
		int[] value = new int[this.getRemainingBytes() - 1];
		 ...
		 log.log(Level.FINEST,"There are " + value.length + " remaining bytes");
