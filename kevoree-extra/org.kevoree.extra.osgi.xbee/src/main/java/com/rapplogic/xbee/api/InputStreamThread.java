...
public class InputStreamThread implements Runnable {
	...
	private final static Logger log = Logger.getLogger(InputStreamThread.class.getName());
...
//		executor.submit(this);
		...
		thread.start();
		 ...
		 log.log(Level.FINEST,"starting packet parser thread");
	}
						...
						try {
							if (pl != null) {
								pl.processResponse(response);	
								 ...
								 log.log(Level.WARNING,"PacketListener is null, size is " + packetListenerList.size());
							}
						} catch (Throwable th) {
							 log.log(Level.WARNING,"Exception in packet listener", th);
						}
					}			
				}				
			}
	...
	private void addToResponseQueue(final XBeeResponse response) throws InterruptedException{
		...
		while (responseQueue.size() >= conf.getMaxQueueSize()) {
			 log.log(Level.FINEST,"Response queue has reached the maximum size of " + conf.getMaxQueueSize() + " packets.  Trimming a packet from head of queue to make room");
			responseQueue.poll();
		}
			...
			while (!done) {
				try {
					if (connection.getInputStream().available() > 0) {
						 log.log(Level.FINEST,"About to read from input stream");
						val = connection.getInputStream().read();
						 log.log(Level.FINEST,"Read " + ByteUtils.formatByte(val) + " from input stream");
						...
						if (val == XBeePacket.SpecialByte.START_BYTE.getValue()) {
							...
							if (log.isLoggable(Level.INFO)) {
								 log.log(Level.INFO,"Received packet from XBee: " + response);
//								log.debug("Received packet: int[] packet = {" + ByteUtils.toBase16(response.getRawPacketBytes(), ", ") + "};");	
							...
							this.addResponse(response);
							 ...
							 log.log(Level.WARNING,"expected start byte but got this " + ByteUtils.toBase16(val) + ", discarding");
						}
					} else {
						 log.log(Level.FINEST,"No data available.. waiting for new data event");
					...
					if (e instanceof IOException) {
						// this is thrown by RXTX if the serial device unplugged while we are reading data; if we are waiting then it will waiting forever
						 log.log(Level.SEVERE,"Serial device IOException.. exiting");
						break;
					}
				}
			}
			...
			if (listenerPool != null) {
				try {
					listenerPool.shutdownNow();
					 ...
					 log.log(Level.WARNING,"Failed to shutdown listner thread pool", t);
				}
			}
		}
		...
		if (thread != null) {
			try {
				thread.interrupt();	
				 ...
				 log.log(Level.WARNING,"Error interrupting parser thread", e);
			}
		}
	}
}