...
public class RxResponseIoSample extends RxBaseResponse implements NoRequestResponse {
	...
	private final static Logger log = Logger.getLogger(RxResponseIoSample.class.getName());
	...
	public void parse(IPacketParser parser) throws IOException {
		...
		super.parseBase(parser);
		 ...
		 log.log(Level.FINEST,"this is a I/O sample!");
		...
		this.setChannelIndicator1(parser.read("Channel Indicator 1"));
		 ...
		 log.log(Level.FINEST,"channel indicator 1 is " + ByteUtils.formatByte(this.getChannelIndicator1()));
		
		// channel indicator 2 (dio)
		this.setChannelIndicator2(parser.read("Channel Indicator 2"));
		
		 log.log(Level.FINEST,"channel indicator 2 is " + ByteUtils.formatByte(this.getChannelIndicator2()));
		
		// collect each sample
		for (int i = 0; i < this.getSamples().length; i++) {
			
			 log.log(Level.FINEST,"parsing sample " + (i + 1));
	...
	private IoSample parseIoSample(IIntInputStream parser) throws IOException {
		...
		if (this.containsDigital()) {
			...
			// next two bytes are DIO
			 ...
			 log.log(Level.FINEST,"Digital I/O was received");
		...
		if (this.containsAnalog()) {
			...
			// adc is 10 bits
			 ...
			 log.log(Level.FINEST,"Analog input was received");
			...
			if (this.isA5Enabled()) {
			 ...
			 log.log(Level.FINEST,"There are " + analog + " analog inputs turned on");
		}
