...
public class XBee implements IXBee {
	...
	private final static Logger log = Logger.getLogger(XBee.class.getName());
	...
	private void doStartupChecks() throws XBeeException {
		...
		try {				
			...
			if (ap.getValue()[0] != 2) {
				 log.log(Level.WARNING,"XBee radio is in API mode without escape characters (AP=1).  The radio must be configured in API mode with escape bytes (AP=2) for use with this library.");
			...
			if (ap.isOk()) {
				...
				case 0x1a:
					...
					break;
				default:
					 log.log(Level.WARNING,"Unknown radio type (HV): " + ap.getValue()[0]);
				}
			}	
		...
		try {
			while (true) {
				...
				if (waitTime <= 0) {
				 ...
				 log.log(Level.FINEST,"calling getResponse with waitTime: " + waitTime);
				...
				if (log.isLoggable(Level.FINEST)) {
					 log.log(Level.FINEST,"Got response in " + (System.currentTimeMillis() - callStart));
				}
				
				responseList.add(response);
				
				if (terminator != null && terminator.stop(response)) {
					 log.log(Level.FINEST,"Found terminating response.. exiting");
					break;
				}
			}			
		 ...
		 log.log(Level.FINEST,"Time is up.. returning list with " + responseList.size() + " packets");
	...
	public void close() {		
		...
		try {
			this.xbeeConnection.getOutputStream().close();
			 ...
			 log.log(Level.FINEST,"Failed to close output stream", e);
		}
