...
public class XBeePacket {
	...
	private final static Logger log = Logger.getLogger(XBeePacket.class.getName());
...
//		}
		...
		if (log.isLoggable(Level.FINEST)) {
			...
			for (int i = 0; i < packet.length; i++) {
			 ...
			 log.log(Level.FINEST,stringBuilder.toString());

			 log.log(Level.FINEST,"pre-escape packet size is " + preEscapeLength + ", post-escape packet size is " + packet.length);
		}
	...
	private static int[] escapePacket(int[] packet) {
		...
		for (int i = 1; i < packet.length; i++) {
			if (isSpecialByte(packet[i])) {
				 log.log(Level.FINEST,"escapeFrameData: packet byte requires escaping byte " + ByteUtils.toBase16(packet[i]));
				escapeBytes++;
			}
		}
		...
		if (escapeBytes == 0) {
			return packet;
			 ...
			 log.log(Level.FINEST,"packet requires escaping");
			...
			for (int i = 1; i < packet.length; i++) {
				if (isSpecialByte(packet[i])) {
					...
					escapePacket[++pos] = 0x20 ^ packet[i];
					 ...
					 log.log(Level.FINEST,"escapeFrameData: xor'd byte is 0x" + Integer.toHexString(escapePacket[pos]));
				} else {
