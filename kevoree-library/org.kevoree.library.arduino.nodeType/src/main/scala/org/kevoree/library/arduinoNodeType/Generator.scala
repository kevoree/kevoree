...
object Generator {
  ...
  def generate(adaptModel: AdaptationModel, nodeName: String,outputDir : String): Boolean = {
    ...
    try {
      ...
      instAdaptation.foreach(instanceAdaptation => {
          ...
          //CREATE NEW INSTANCE
          val clazzFactory: Class[_] = loader.loadClass(instance.getTypeDefinition.getFactoryBean.replaceAll("Factory", "Activator"))
          ...
          instance match {
            case c: Channel => {
                var typeDef = c.getTypeDefinition.asInstanceOf[ChannelType]
                //INIT BINDINGS
                var channelInstance = reflectiveInstance.asInstanceOf[ChannelTypeFragment]
