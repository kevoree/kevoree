...
class DataManagerForGroup(nameInstance : String,selfNodeName : String,modelService : KevoreeModelHandlerService) extends DataManager[String] with actors.DaemonActor {
  ...
  case class SetData(uuid : UUID, tuple : Tuple2[VectorClock,String])
  ...
  def getData(uuid : UUID) : Tuple2[VectorClock,String] ={
	(this !? GetData(uuid)).asInstanceOf[Tuple2[VectorClock,String]]
  }
  
  def setData(uuid : UUID, tuple : Tuple2[VectorClock,String]) ={
	  ...
	  react {
		...
		case GetData(uuid) => {
			if (uuid.equals(this.uuid)) {
			  ...
			  var tuple = new Tuple2[VectorClock,String](vectorClock,stringFromModel)
			  reply(tuple) // TODO maybe we need to clone
			} else {
			  reply{}
			}
		  }
		...
		case RemoveData(uuid) => {}//datas.remove(uuid) // We do nothing because the model cannot be deleted
		...
		case MergeClock(uuid,newClock)=> {
			var mergedVC = localMerge(newClock)
  ...
  private def stringFromModel() : String ={
	var out = new ByteArrayOutputStream
	KevoreeXmiHelper.saveStream(out, modelService.getLastModel())
	out.flush()
	var s =out.toString("UTF8")
	out.close
	s
  }
  
  private def modelFromString(modelString : String) : ContainerRoot ={
	var stream = new ByteArrayInputStream(modelString.getBytes("UTF8"))
	KevoreeXmiHelper.loadStream(stream)
  }
  
  
  private def updateOrResolve(tuple : Tuple2[VectorClock,String]) ={
	var model = modelFromString(tuple._2)
	var occured = VersionUtils.compare(vectorClock, tuple._1)
	occured match {
	  case Occured.AFTER=> {}
	  case Occured.BEFORE=> {
	  ...
	  case Occured.CONCURRENTLY=> {
		  var localDate = new Date(vectorClock.getTimestamp());
		  var remoteDate = new Date(tuple._1.getTimestamp());
		  //TODO TO IMPROVE
  ...
  private def increment():VectorClock={
	...
	var selfFound = false;
	vectorClock.getEntiesList.foreach{clock=>
