...
class GossiperRequestReceiver (protected var channelFragment: NettyGossipAbstractElement, dataManager: DataManager,
  port: Int, gossiperRequestSender: GossiperRequestSender, fullUDP: java.lang.Boolean, serializer: Serializer)
  ...
  protected var factoryForRequest = new NioDatagramChannelFactory(Executors.newCachedThreadPool())
  protected var bootstrapForRequest = new ConnectionlessBootstrap(factoryForRequest)
  ...
  protected var channel: Channel = null //bootstrapForRequest.bind(new InetSocketAddress(port));

  protected var factoryForRequestTCP = new
      NioServerSocketChannelFactory(Executors.newCachedThreadPool(), Executors.newCachedThreadPool())
  protected var bootstrapForRequestTCP = new ServerBootstrap(factoryForRequestTCP)
  ...
  protected var channelTCP: Channel = null //bootstrapForRequestTCP.bind(new InetSocketAddress(port))
  ...
  private def doGossip (message: Message, address: InetSocketAddress, channel: Channel) /*: Channel*/ = {
    //println(address)
    var responseBuilder: Message.Builder = Message.newBuilder.setDestName(channelFragment.getName)
    ...
    message.getContentClass match {
      case s: String if (s == classOf[VectorClockUUIDsRequest].getName) => {
        ...
        uuidVectorClocks.keySet.foreach {
          uuid: UUID =>
            vectorClockUUIDsBuilder.addVectorClockUUIDs(Gossip.VectorClockUUID.newBuilder.setUuid(uuid.toString)
            ...
            if (vectorClockUUIDsBuilder.getVectorClockUUIDsCount == 1) {
              ...
              responseBuilder = Message.newBuilder.setDestName(channelFragment.getName)
                .setDestNodeName(channelFragment.getNodeName)
      ...
      case s: String if (s == classOf[UUIDDataRequest].getName) => {
        ...
        if (bytes != null) {
          ...
          val modelBytes2 = Gossip.VersionedModel.newBuilder.setUuid(uuidDataRequest.getUuid).setVector(data._1)
