...
public class PaaSKloudResourceManagerPage extends ParentAbstractPage {
	...
	public KevoreeHttpResponse process (KevoreeHttpRequest request, KevoreeHttpResponse response) {
		if (request != null) {
			if (request.getUrl().equals(this.getDictionary().get("urlpattern").toString())) {
				...
				response = processStatusPage(request, response);
			} else {
	...
	*/
	...
	private KevoreeHttpResponse processKloudRequest (KevoreeHttpRequest request, KevoreeHttpResponse response) {
		if (request.getResolvedParams().get("model") != null && request.getResolvedParams().get("login") != null
			...
			if (InriaLdap.testLogin(request.getResolvedParams().get("login"), request.getResolvedParams().get("password"))) {
				// log in is done, now we try to submit user configuration		
//				response.setContent(process(request.getResolvedParams().get("model"), request.getResolvedParams().get("login"), request.getResolvedParams().get("ssh_key")));
				...
				if (KloudHelper.lookForAGroup(request.getResolvedParams().get("login"), this.getModelService().getLastModel())) {

					// if the user has already submitted something, we return the access point to this previous configuration
					Option<String> accessPointOption = KloudHelper.lookForAccessPoint(request.getResolvedParams().get("login"), this.getNodeName(), this.getModelService().getLastModel());
					if (accessPointOption.isDefined()) {
						msg = HTMLHelper.generateUnvalidSubmissionPageHtml(request.getResolvedParams().get("login"),
								"A previous configuration has already submitted.<br/>Please use this access point to reconfigure it: "
										+ accessPointOption.get() + "(" + accessPointOption.get().replace("http://", "").replace("/model/current", "") + " on the editor)"
										+ "<br />This access point allow you to access to a Kevoree group that allows you to send a model to it."
										+ "<br />This model will be used to reconfigure your nodes and add or remove some of them if necessary.", this.getDictionary().get("urlpattern").toString());
					} else {
					...
					message.putValue("model", request.getResolvedParams().get("model"));
					if (request.getResolvedParams().get("ssh_key") != null && !request.getResolvedParams().get("ssh_key").equals("")) {
						message.putValue("sshKey", request.getResolvedParams().get("ssh_key"));
					}
					getPortByName("deploy", MessagePort.class).process(message);
					msg = HTMLHelper.generateRedirect(request.getResolvedParams().get("login"), this.getDictionary().get("urlpattern").toString());
				}
			...
			// return a form to submit something
	...
	private KevoreeHttpResponse processCSS (KevoreeHttpRequest request, KevoreeHttpResponse response) {
		try {
	...
	private KevoreeHttpResponse processStatusPage (KevoreeHttpRequest request, KevoreeHttpResponse response) {
	...
	private String findAddress (String login) { // Here we may had the proxy page
