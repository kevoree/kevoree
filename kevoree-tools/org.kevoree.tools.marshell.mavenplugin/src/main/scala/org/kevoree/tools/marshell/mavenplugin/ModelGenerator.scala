...
object ModelGenerator extends App {
  ...
  def generate (scriptPath: String, mavenSource: MavenProject): ContainerRoot = {
    ...
    /*System.getProperties.keySet().foreach {
      key => kevEngine.addVariable(key.toString, System.getProperty(key.toString).toString);
    }*/
    if (properties != null) {
      properties.keySet().foreach {
        key => kevEngine.addVariable(key.toString, properties.get(key.toString).toString);
      }
    }
