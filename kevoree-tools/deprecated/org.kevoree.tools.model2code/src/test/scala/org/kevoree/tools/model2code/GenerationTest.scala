...
object GenerationTest extends App {
  ...
  private val kevoreeFactory = new DefaultKevoreeFactory
  ...
  private def generateModel(nbDu: Int, nbCtInDu: Int, typesName: String, model:ContainerRoot): ContainerRoot = {
    for (i <- 0 to nbDu-1) {
      ...
      du.setName("org.entimid.genlib." + typesName + i)
  ...
  private def generateMasterPom(model: ContainerRoot) {
    ...
    deployUnit.setName("org.entimid.genlib.root")
    ...
    model.getDeployUnits.foreach {
      du =>
        pr.println("    <module>../" + du.getName + "</module>")
    }
    pr.println("  </modules>")
    pr.println("")
    pr.println("</project>")
  ...
  model = generateModel(100,1,"C3TestComponent", model)
    ...
    model.getDeployUnits.foreach {
      du => generator.modelToDeployUnit(model, URI.create("file:/Users/gnain/sources/entimid/entimid-library/genlibs/" + du.getName), du)
    }
