...
case class ComponentTypeWorker(root: ContainerRoot, _componentType: org.kevoree.ComponentType, _compilationUnit: CompilationUnit)
  ...
  def synchronize() {
    ...
    synchronizeStart(td, componentType.getStartMethod)
    synchronizeStop(td, componentType.getStopMethod)
    synchronizeUpdate(td, componentType.getUpdateMethod)
    synchronizeProvidedPorts(td)
    synchronizeRequiredPorts(td)
    synchronizeLibrary(root, td, componentType.getName)
    synchronizeDictionary(td, componentType)
  ...
  private def sychronizeClass: TypeDeclaration = {
    ...
    val td = compilationUnit.getTypes.find({
      typ => typ.getName.equals(componentType.getName)
    }) match {
      case Some(td: TypeDeclaration) => {
        System.out.print("Synchronizing " + componentType.getName + "...")
        td
      }
      case None => {
        System.out.print("Generating " + componentType.getName + "...")
        //Class creation
