 ...
 */
 ...
 /*
  ...
  16 Janvier 2012 - add param to triggered function jedartois@gmail.com
*/
...
AlarmClass::AlarmClass()
{
  ...
  value = nextTrigger = 0;
  onTickHandler.fnc = NULL;  // prevent a callback until this pointer is explicitly set 
 onTickHandler.id = 0; 
}
...
AlarmID_t TimeAlarmsClass::alarmRepeat(time_t value, OnTick_t onTickHandler){ // trigger daily at the given time
    ...
    {
      if(isAllocated(ID)) {
        Alarm[ID].Mode.isEnabled = (Alarm[ID].value != 0) && (Alarm[ID].onTickHandler.fnc != 0) ;  // only enable if value is non zero and a tick handler has been set
    ...
    void TimeAlarmsClass::free(AlarmID_t ID)
    {
      if(isAllocated(ID))
      {
        ...
        Alarm[ID].onTickHandler.fnc = 0;
  Alarm[ID].onTickHandler.id = 0;
    	Alarm[ID].value = 0;
      ...
      if(! isServicing)
      {
        ...
        for( servicedAlarmId = 0; servicedAlarmId < dtNBR_ALARMS; servicedAlarmId++)
        {
          if( Alarm[servicedAlarmId].Mode.isEnabled && (now() >= Alarm[servicedAlarmId].nextTrigger)  )
          {
            ...
            if( TickHandler.fnc != NULL) {       
              (*(TickHandler.fnc))(TickHandler.id);     // call the handler  
            }
          }
        }
        isServicing = false;
      ...
      {  
    	for(uint8_t id = 0; id < dtNBR_ALARMS; id++)
        {
    	  ...
    	  // here if there is an Alarm id that is not allocated
      	    Alarm[id].onTickHandler.fnc =  onTickHandler.fnc;
	     Alarm[id].onTickHandler.id = onTickHandler.id;
