...
import org.kevoree.PhysicalNodeType

case class PhysicalNodeTypeVisitor(physicalNodeType : PhysicalNodeType,env : AnnotationProcessorEnvironment)
extends SimpleDeclarationVisitor 
  ...
  override def visitClassDeclaration(classdef : ClassDeclaration) {
    //SUB PROCESSOR
    processDictionary(physicalNodeType,classdef)
    processDeployUnit(physicalNodeType,classdef,env)
    processLibrary(physicalNodeType,classdef)
    processThirdParty(physicalNodeType,classdef,env)
    classdef.getMethods.foreach{method => method.accept(this) }
  }

  override def visitMethodDeclaration(methoddef : MethodDeclaration) {
    processLifeCycleMethod(physicalNodeType,methoddef)
  }
