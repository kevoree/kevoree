...
class AndroidKevoreeJarClassLoader(gkey: String, ctx: android.content.Context, parent: ClassLoader) extends KevoreeJarClassLoader {
  ...
  def declareLocalDexClassLoader(dexStream: InputStream, idName: String) {
    ...
    if (idName.contains("classes.dex")) {
      newDexCL = new KevoreeDexClassLoader(getLoadedURLs.get(0).getPath, outDex.getAbsolutePath, null, this)
    } else {
      ...
      newDexCL = new KevoreeDexClassLoader(dexInternalStoragePath.getAbsolutePath, outDex.getAbsolutePath /*dexOptStoragePath.getAbsolutePath*/ , null, this)
    }
