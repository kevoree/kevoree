...
const prog_char ultrasonicrange[] PROGMEM = "UltraSonicRange";
...
const prog_char port_range[] PROGMEM = "range";
PROGMEM const char * portdefinition[] = { port_range};
const prog_char prop_dpin[] PROGMEM = "dpin";
const prog_char prop_period[] PROGMEM = "period";
PROGMEM const char * properties[] = { prop_dpin,prop_period};
#include <EEPROM.h>
...
class UltraSonicRange : public KevoreeType {
 public : 
...
char period[20];
void init(){
range = (kbinding*) malloc(sizeof(kbinding));
if(range){
   memset(range, 0, sizeof(kbinding));
}
...
cm = ( duration / 29 / 2 );
kmessage * smsg = (kmessage*) malloc(sizeof(kmessage));if (smsg){memset(smsg, 0, sizeof(kmessage));}sprintf(buf,"%d",int(cm));
smsg->value = buf;
smsg->metric="cm";range_rport(smsg);free(smsg);
nextExecution += atol(period);
}
void range_rport(kmessage * msg){
if(range->port){
range->port->push(*msg);
}
}
...
for(int i=0;i<bindings->nbBindings;i++){    bindings->bindings[i]->port->push(*msg);}Serial.print(instanceName);Serial.print(":");Serial.print("kduke");Serial.print("[");Serial.print(msg->metric);Serial.print("/");Serial.print(msg->value);Serial.println("]");
}
};
...
case 0:{
((UltraSonicRange*) instances[index])->destroy();
break;}
case 1:{
...
void updateParam(int index,int typeCode,int keyCode,char * val){
 switch(typeCode){
case 0:{
UltraSonicRange * instance = (UltraSonicRange*) instances[index];
 switch(keyCode){
case 0:{
strcpy (instance->dpin,val);
break;}
case 1:{
...
switch(typeCode){
case 0:{
  UltraSonicRange * newInstance = (UltraSonicRange*) malloc(sizeof(UltraSonicRange));
  if (newInstance){
    memset(newInstance, 0, sizeof(UltraSonicRange));
  } 
...
case 0:{
UltraSonicRange * instance = (UltraSonicRange*) instances[indexComponent];
switch(portCode){
case 0:{
   requiredPort=instance->range;
break;}
}
break;}
}
...
switch(portCode){
case 0:{
   requiredPort=(((UltraSonicRange*)instances[indexComponent])->range);
break;}
}
break;}
}
...
case 0:{
return millis() > (((UltraSonicRange *) instances[index] )->nextExecution);
}
}
return false;
}
...
for(int i=0;i<strlen(((UltraSonicRange *) instances[instanceIndex])->dpin);i++){
save2Memory(((UltraSonicRange *) instances[instanceIndex])->dpin[i]);
}
save2Memory(',');
save2Memory(1);
save2Memory(delimsEQ[0]);
for(int i=0;i<strlen(((UltraSonicRange *) instances[instanceIndex])->period);i++){
save2Memory(((UltraSonicRange *) instances[instanceIndex])->period[i]);
}
break;
}
}
...
eepromIndex--;
  ...
  void saveInstancesBindings(int instanceIndex){      
   switch(instances[instanceIndex]->subTypeCode){     
case 0:{
                  if ((((UltraSonicRange *) instances[instanceIndex]) -> range -> port))                          
                  {                                                                                    
                    saveBI_CMD(                                                                        
                      true,                                                                            
                      instances[instanceIndex] -> instanceName,                                        
                      ((UltraSonicRange *) instances[instanceIndex]) -> range -> instance -> instanceName,        
                    ((UltraSonicRange *) instances[instanceIndex]) -> range -> portCode                           
                    );                                                                                 
                  }                                                                                    
break;
}
...
const prog_char init_serialct561[] PROGMEM = "SerialCT561";
const prog_char init_ultrasoni546[] PROGMEM = "UltraSoni546";
PROGMEM const char * init_tables[] = {init_serialct561,init_ultrasoni546};
...
save2Memory(sepAdminChar);
        ...
        saveAIN_CMD(instNameBuf, 0, "period=100,dpin=0");                                             
save2Memory(sepAdminChar);
strcpy_P(instNameBuf, (char *) pgm_read_word (&(init_tables[1])));        
...
long nextExecutionGap(int index){
 switch(instances[index]->subTypeCode){
case 0:{
return ((((UltraSonicRange *) instances[index] )->nextExecution)- currentMillis());
}
}
return -1;
}
