...
public class ForkedGregServer {
    ...
    public static void startServer(String[] args, final File output) {
        ...
        final ForkedGregServer server = new ForkedGregServer(conf);
        ...
        new Thread() {
            public void run() {
                try {
                    ...
                    OutputStream os = new BufferedOutputStream(new FileOutputStream(output), 16384);
    ...
    public ForkedGregServer(Configuration conf) {
        this.conf = conf;
