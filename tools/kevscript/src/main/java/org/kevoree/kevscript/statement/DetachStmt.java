        ...
        if (groupList.size() == 1) {
            if (groupList.get(0).equals("*")) {
                ...
                if (group != null) {
                    ...
                    throw new KevScriptException("Unable to find group instance \""+groupList.get(0)+"\". Detach failed");
                }
            }
        } else {
            throw new KevScriptException("Detach target path is invalid ("+groupList.toString()+")");
        }
            ...
            if (instancePath.size() == 1) {
                if (instancePath.get(0).equals("*")) {
                    ...
                    if (node != null) {
                        ...
                        throw new KevScriptException("Unable to detach node instance \""+instancePath.get(0)+"\". Instance does not exist");
                    }
                }
            } else {
                throw new KevScriptException("\""+instancePath.toString()+"\" is not a valid detach path for a node");
            }
        }
    }
